...
void main() {
  ...
  test('an install-config link reports its url', () async {
    ...
    await emit('forkfc://install-config?url=https://example.com/a.yaml');
  ...
  test('an install-config link without a url is ignored', () async {
    ...
    await emit('forkfc://install-config');
  ...
  test('a link for another host is ignored', () async {
    ...
    await emit('forkfc://open-profile?url=https://example.com/a.yaml');
  ...
  test(
    ...
    () async {
      linkManager.seedInitialLink([
        '--verbose',
        'forkfc://install-config?url=https://example.com/a.yaml',
      ]);

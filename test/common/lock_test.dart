...
void main() {
  ...
  setUp(() {
    home = Directory.systemTemp.createTempSync('forkfc-lock-');
    SingleInstanceLock.resolvePath = () async => join(home.path, 'ForkFC.lock');
  });
  ...
  test(
    ...
    () async {
      final path = join(home.path, 'ForkFC.lock');
  ...
  test('an unusable lock path is reported, not thrown', () async {
    SingleInstanceLock.resolvePath = () async =>
        join(home.path, 'missing-dir', 'ForkFC.lock');
  ...
  test('a path that is a directory is reported, not thrown', () async {
    final directory = Directory(join(home.path, 'ForkFC.lock'))

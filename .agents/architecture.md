...
to a running core; `ForkFCHttpOverrides` sends the credentials with the app's own proxied requests; and `sharedState`
...
- Android may create an Always-on `VpnService` through `onStartCommand()` without ForkFC's bound-service path. The service
...
the `chen08209/flutter_distributor` fork pinned to a `v<version>-forkfc.<n>` tag (cut a new tag there and bump
...
- macOS: a standalone `ForkFCCore`. `Release.xcconfig` pins release and profile `ARCHS` to the host because
...
- Linux and Windows: `ForkFCCore`, the Rust `ForkFCHelperService`, and a `manifest.json` holding `coreSha256`; the
...
- `/ping` is loopback-only and requires no request token. The Helper compares the requested SHA256 with its embedded value
  and checks that the fixed `ForkFCCore.exe` beside it exists; `/start` performs the actual Core SHA256 verification before
...
- Flutter creates a 128-bit lowercase-hex session ID and uses it as the random named-pipe suffix. `/start` receives only
  that address and session ID, validates the fixed `ForkFCCore_<session>` namespace, starts the fixed Core beside the
...
Architecture detection is automatic. The `--description` flag passed to `flutter_distributor` adds arch suffixes to artifact names, such as `ForkFC-0.8.93-macos-arm64.dmg`.
...
- `tray`: system tray for Linux, macOS and Windows. Written for ForkFC; replaced the `tray_manager` fork.
...
- `ForkFCHelperService.exe install` stops and removes any stale registration, creates the auto-start service for the
  current executable path, starts it, and waits for the running state.
- `ForkFCHelperService.exe uninstall` stops the service, waits for shutdown, removes its registration, and is also used
...
- `ForkFCHelperService install`, run through `pkexec` so polkit raises the system prompt, writes
  `/etc/systemd/system/forkfc-helper.service` for the current executable path and enables and restarts it. It reads
...
- That ownership check is why the `flutter_distributor` fork normalizes the packaging tree to 0755/0644 before
  ...
  of 002 would otherwise ship `/opt/ForkFC` as 0775, which the installer rejects as group-writable.
- The rpm spec sets `debug_package` and `__os_install_post` to nil for the same reason: rpmbuild's find-debuginfo and
  brp-strip rewrite `ForkFCCore`, and a Core whose SHA256 no longer matches the Helper's embedded value is refused at
  `/start`, which silently degrades every launch to the direct Core.
- `ForkFCHelperService uninstall` disables the unit, removes it and reloads systemd.
- The unit carries `Group=` (the owner's primary GID), `RuntimeDirectory=forkfc`, the owner's UID/GID in
  `FORKFC_HELPER_OWNER_UID`/`_GID`, a double-quoted `ExecStart=` with `%` escaped, and `Restart=on-failure` under a
  start limit so a broken unit ends up failed instead of restarting forever. The helper serves
  `/run/forkfc/helper.sock` at mode `0660`, additionally drops any connection whose `SO_PEERCRED` UID is not the
...
- `GET /ping?coreSha256=...` returns the current Helper executable path with `x-forkfc-helper-protocol` when the
...
Endpoints bind only to `127.0.0.1:47890` on Windows and to `/run/forkfc/helper.sock` on Linux, and do not use
request-token authentication. Lifecycle safety comes from the fixed executable/hash, the strict address namespace
(`\\.\pipe\ForkFCCore_<32 hex>` on Windows, `/tmp/ForkFCSocket_<digits>.sock` on Linux), the session-scoped stop

...
- macOS and Linux build a standalone `ForkFCCore` process used by the desktop socket integration.
- Windows builds `ForkFCCore.exe`, the Rust `ForkFCHelperService.exe` privileged helper, and a
...
FFI library, while setup is only the build and packaging bridge for ForkFC's external core artifacts.

...
ForkFC is a multi-platform proxy client based on ClashMeta (mihomo), built with Flutter. It supports Android, Windows, macOS, and Linux, using a Material You design with Surfboard-like UI.
...
version 0.5.1, pinned to the tag `v0.5.1-forkfc.1` because the fork carries
...
- This one is not optional while it lasts: ForkFC depends on `win32_registry: ^3.0.3`

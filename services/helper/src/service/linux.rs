...
const SERVICE_NAME: &str = "forkfc-helper";
const UNIT_PATH: &str = "/etc/systemd/system/forkfc-helper.service";
const RUNTIME_DIR_NAME: &str = "forkfc";
const SOCKET_PATH: &str = "/run/forkfc/helper.sock";
const OWNER_UID_ENV: &str = "FORKFC_HELPER_OWNER_UID";
const OWNER_GID_ENV: &str = "FORKFC_HELPER_OWNER_GID";
...
fn unit_contents(executable: &Path, owner: Owner) -> String {
    format!(
        "[Unit]\n\
         Description=ForkFC Helper starts the ForkFC Core with the privileges TUN mode needs.\n\
    ...
    match installed_owner_uid(&existing) {
        Some(uid) if uid != owner.uid => bail!(
            "the Helper is already installed for UID {uid}; \
             run `ForkFCHelperService uninstall` as that user first"
        ),
        _ => Ok(()),
    }
}
...
mod tests {
    ...
    fn unit_names_the_owner_and_the_helper_it_starts() {
        let unit = unit_contents(
            Path::new("/opt/ForkFC/ForkFCHelperService"),
            Owner {
                uid: 1000,
                gid: 1001,
            },
        ...
        assert!(unit.contains("ExecStart=\"/opt/ForkFC/ForkFCHelperService\"\n"));
        assert!(unit.contains("Group=1001\n"));
        assert!(unit.contains("Environment=FORKFC_HELPER_OWNER_UID=1000\n"));
        assert!(unit.contains("Environment=FORKFC_HELPER_OWNER_GID=1001\n"));
        assert!(unit.contains("RuntimeDirectory=forkfc\n"));

...
#[cfg(not(target_os = "linux"))]
const CORE_PIPE_PREFIX: &str = r"\\.\pipe\ForkFCCore_";
#[cfg(target_os = "linux")]
const CORE_SOCKET_PREFIX: &str = "/tmp/ForkFCSocket_";
#[cfg(target_os = "linux")]
const CORE_SOCKET_SUFFIX: &str = ".sock";
const PROTOCOL_VERSION_HEADER: &str = "x-forkfc-helper-protocol";
...
mod tests {
    ...
    #[cfg(not(target_os = "linux"))]
    const ALLOWED_CORE_ADDRESS: &str = r"\\.\pipe\ForkFCCore_0123456789abcdef0123456789abcdef";
    #[cfg(target_os = "linux")]
    const ALLOWED_CORE_ADDRESS: &str = "/tmp/ForkFCSocket_4821.sock";
    ...
    async fn ping_returns_running_helper_path_for_verified_core() {
        let response = ping_response(Ok(PathBuf::from("ForkFCHelperService.exe")));
    ...
    fn verifies_core_sha256_in_all_build_modes() {
        let path =
            std::env::temp_dir().join(format!("forkfc-helper-core-sha256-{}", std::process::id()));
    ...
    fn only_accepts_random_core_pipe_namespace() {
        assert!(is_allowed_core_address(
            r"\\.\pipe\ForkFCCore_0123456789abcdef0123456789abcdef"
        ));
        assert!(!is_allowed_core_address(r"\\.\pipe\ForkFCCore"));
        assert!(!is_allowed_core_address(
            r"\\.\pipe\Other_0123456789abcdef0123456789abcdef"
        ));
        assert!(!is_allowed_core_address(
            r"\\.\pipe\ForkFCCore_0123456789abcdef"
        ));
        assert!(!is_allowed_core_address(
            r"\\.\pipe\ForkFCCore_0123456789abcdef0123456789abcdeg"
        ));
        assert!(!is_allowed_core_address(
            r"\\.\pipe\ForkFCCore_ABCDEF0123456789abcdef0123456789"
        ));
    }
    ...
    fn only_accepts_random_core_socket_namespace() {
        assert!(is_allowed_core_address("/tmp/ForkFCSocket_4821.sock"));
        assert!(!is_allowed_core_address("/tmp/ForkFCSocket_.sock"));
        assert!(!is_allowed_core_address("/tmp/ForkFCSocket_4821"));
        assert!(!is_allowed_core_address("/tmp/Other_4821.sock"));
        assert!(!is_allowed_core_address("/tmp/ForkFCSocket_../x.sock"));
        assert!(!is_allowed_core_address(
            "/tmp/ForkFCSocket_12345678901.sock"
        ));
    }

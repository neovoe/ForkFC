...
mod tests {
    ...
    async fn start_rejects_a_caller_supplied_core_argument() {
        let response = warp::test::request()
            ...
            .body(
                r#"{"address":"\\\\.\\pipe\\ForkFCCore_0123456789abcdef0123456789abcdef","sessionId":"0123456789abcdef0123456789abcdef","path":"attacker.exe"}"#,
            )
            .reply(&routes())
            .await;
    ...
    async fn start_releases_the_managed_core_before_rejecting_an_unverified_core() {
        ...
        *MANAGED_CORE.lock().unwrap() = Some(ManagedCore {
        ...
        let response = warp::test::request()
            ...
            .path("/start")
            .json(&StartParams {
                address: r"\\.\pipe\ForkFCCore_0123456789abcdef0123456789abcdef".to_string(),
    ...
    fn only_accepts_random_core_pipe_namespace() {
        ...
        assert!(!is_allowed_core_pipe(r"\\.\pipe\ForkFCCore"));
        assert!(!is_allowed_core_pipe(

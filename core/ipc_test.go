...
func TestDecodeMethodArgumentsAcceptsValidPayload(t *testing.T) {
	call := &MethodCall{
		ID:        "1",
		Method:    initClashMethod,
		Arguments: json.RawMessage(`{"home-dir":"/tmp/forkfc","version":3}`),
	}
	target := InitParams{}
	...
	if target.HomeDir != "/tmp/forkfc" || target.Version != 3 {
		t.Errorf("decoded params = %+v, want {/tmp/forkfc 3}", target)
	}
}
